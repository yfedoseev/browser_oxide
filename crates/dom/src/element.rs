...
use css_selectors::Element;

/// A DOM element wrapper that implements `css_selectors::Element`.
///
...
impl<'a> DomElement<'a> {
    ...
    fn element_data(&self) -> &crate::node::ElementData {
...
mod tests {
    use super::*;
    use crate::arena::Dom;
    use crate::node::{Attribute, QualName};
    ...
    fn selector_matching_integration() {
        ...
        let selectors = css_selectors::parse_selector_list("div#main.container").unwrap();
        assert!(css_selectors::matches_selector(&div_el, &selectors[0]));

        let selectors2 = css_selectors::parse_selector_list("body > div").unwrap();
        assert!(css_selectors::matches_selector(&div_el, &selectors2[0]));

        let selectors3 = css_selectors::parse_selector_list(".nonexistent").unwrap();
        assert!(!css_selectors::matches_selector(&div_el, &selectors3[0]));
    }
}
