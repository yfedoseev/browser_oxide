...
fn biquad_coeffs(
    ...
    let alpha_s = sin_w0 / 2.0 * ((a_factor + 1.0 / a_factor) * (1.0 / 1.0 - 1.0) + 2.0).sqrt();
    let two_sqrt_a_alpha = 2.0 * a_factor.sqrt() * alpha_s;
    let _ = two_sqrt_a_alpha; // unused for shelf (spec uses a different alpha)
