...
- Called only by other `#[cfg(test)]` code in `crates/browser_oxide/tests/ja4h_*.rs`.
...
- Re-export `ja4h::ja4h()` from `crates/browser_oxide/src/net/mod.rs` outside `#[cfg(test)]`.
