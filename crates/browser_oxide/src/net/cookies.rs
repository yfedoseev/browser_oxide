...
struct Cookie {
    ...
    /// Fixes mail-ru cookie relay.
    #[serde(default)]
    domain: Option<String>,
    secure: bool,
    #[allow(
        dead_code,
        reason = "parsed cookie http_only attribute retained for completeness; not yet read"
    )]
    http_only: bool,
...
impl CookieJar {
    ...
    /// Total cookie count across all domains (diagnostics).
    ...
    pub fn set_cookies(&mut self, url: &Url, set_cookie_headers: &[String]) {
        ...
        for header in set_cookie_headers {
            if let Some(cookie) = parse_set_cookie(header, &request_domain, url.path()) {
                ...
                // A Set-Cookie whose expiry is in the past
    ...
    /// Used to accumulate challenge-vendor (and other) trust across pipeline runs:
    ...
    /// clearing `twitter.com` before a fresh `x.com` nav prevents the
...
fn parse_http_date(s: &str) -> Option<u64> {
    ...
    const FORMATS: &[&str] = &[
        ...
        // `%y` maps 27 → 2027. (Full-weekday `%A` form below
...
mod tests {
    ...
    fn expires_2digit_year_stored_not_deleted() {
        ...
        // 307-redirect loop never broke. %y must parse 27→2027.
    ...
    // Regression: an Expires-in-the-past
