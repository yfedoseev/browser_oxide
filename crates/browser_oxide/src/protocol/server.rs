...
use crate::protocol::session::{json_escape_string, CdpSession};
    ...
    pub fn start(html: &str, port: u16) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let thread = std::thread::spawn(move || {
            ...
            local.block_on(&rt, async move {
                let page =
                    match crate::Page::from_html(&html, None::<crate::stealth::StealthProfile>)
                        .await
                    {
                        Ok(p) => p,
                        Err(e) => {
        ...
        let thread = std::thread::spawn(move || {
            ...
            local.block_on(&rt, async move {
                ...
                let page = match client.get(&url).await {
                    Ok(resp) => {
                        let html = resp.text();
                        match crate::Page::with_profile(&html, &url, profile).await {
                            Ok(p) => p,
                            Err(e) => {
                                tracing::error!(
    ...
    pub fn start_navigable(port: u16) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let thread = std::thread::spawn(move || {
            ...
            local.block_on(&rt, async move {
                ...
                let page = match crate::Page::from_html(
                    "<html><body></body></html>",
...
#[allow(
    ...
    reason = "single-threaded CDP runtime; RefCell borrow dropped before the await resolves"
)]
...
mod tests {
    ...
    fn bench_page_creation_overhead() {
        ...
        // Warm up
        rt.block_on(async {
            let _ = crate::Page::from_html(html, None::<crate::stealth::StealthProfile>).await;
        });
