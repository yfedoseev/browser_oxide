...
//! per-vendor anti-bot challenges (sensor-payload sensors, `x-kpsdk-*`
//! PoW round-trips, interstitial round-trips, orchestrator runners,
//! etc.).
//!
//! The public engine ships the trait + the dispatch wiring only;
//! concrete per-vendor solver implementations are out of scope for this
//! repository (see `SCOPE.md`). Embedders register the set of solvers
//! they want when constructing a `Page` / `HttpClient`; with an empty
//! set the engine still navigates and renders, it just won't auto-clear
//! challenges.
//!
...
//!    `HttpClient` (for the sensor-payload POST, /tl, /mfc, …). Outcome
//!    drives whether the navigate loop retries, breaks, or surfaces an
//!    incomplete-challenge verdict.
//! 5. `relax_response_csp(&resp, &html)` is consulted before installing
//!    the origin's CSP — `rt:'i'` interstitials need the
//!    origin's 403 CSP suspended so `i.js` can reach
//!    `geo.captcha-delivery.com`.
//! 6. `solved_signal(&cookies, &body)` is consulted by the per-iteration
//!    poll — a sec-cpt cookie flipping to its solved marker lets the engine
...
/// solver can distinguish e.g. a managed challenge from a
/// JS challenge from an interactive widget, without exposing the full vendor
/// type surface to the engine.
...
pub struct ChallengeKind {
    ...
    /// Vendor-internal sub-kind, e.g. "sec-cpt" vs "sensor-data" for one
    /// vendor; "managed" vs "jsch" vs "non-interactive" vs "turnstile"
    /// for another. Free-form; the engine only uses it for telemetry.
...
pub enum SolveOutcome {
    ...
    NotApplicable,
    /// Solver is partway through (e.g. an orchestrator pumping
    /// the event loop, waiting for its clearance cookie). The engine should
...
pub trait ChallengeSolver: Send + Sync {
    ...
    /// Used by `rt:'i'` interstitials whose JS reaches
    ...
    /// per-iteration event-loop poll to break out early (e.g. a
    /// sec-cpt cookie flipping to its solved marker, a vendor
    /// cookie appearing on a non-challenge body).
