...
pub struct WebWorker {
    ...
    #[allow(
        dead_code,
        reason = "RAII: keeps the worker channel receive endpoint alive until the receive path is wired"
    )]
    rx: mpsc::Receiver<Value>,
    #[allow(
        dead_code,
        reason = "RAII: keeps the worker channel send endpoint alive until the receive path is wired"
    )]
