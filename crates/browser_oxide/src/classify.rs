...
/// cover the known small "pass" shells (~7.8 KB i18n splash, ~9.6 KB
/// invisible-v3 shell, ~13 KB unsupported-browser shell). Only affects the coarse
...
/// key string that occurs verbatim in fully-rendered pages — a
/// historical false positive on one such site) and is now size-gated in
...
/// first match wins. FP-B4 adds `_cf_chl_opt` — the inline
/// challenge-options JS object that is present *only* on the
...
/// any-size structural signal that catches the large orchestrator shell
/// `/cdn-cgi/challenge-platform/` alone cannot — that JSD URL also
/// stays on *passed* pages, so it is deliberately NOT used here).
/// AWS-WAF token-challenge markers (`gokuprops` / `awswafcookiedomainlist`)
/// are deliberately NOT here — some sites retain them on their solved page, so
...
/// for some sites (which DROP the AWS-WAF script after solving) but FALSE for
/// others, which KEEP `awswafcookiedomainlist` (and sometimes `gokuProps`)
/// embedded in their fully-rendered, solved multi-hundred-KB React app
/// (a real app global, real listings). Treating these as any-size
/// UNAMBIGUOUS therefore false-FAILED such a site as AWS-WAF-CHL.
///
/// The reliable discriminator (solved app vs unsolved stub) is NOT the
...
/// size-independent, so it cannot false-FAIL a large solved body NOR
...
/// Present on the unsolved stub, gone once the token clears (solved page).
...
/// Bot-detection fingerprint markers that appear in BOTH rendered pages (as
...
const SMALL_BODY: &[(&str, &str)] = &[
    ...
    // anti-bot interstitial hook, but those interstitials are small —
    // gating it `< INTERSTITIAL_MAX_BYTES` keeps true detection while
    // killing the false-positive class (literal `px-captcha` in a multi-MB
    // rendered page's CSS / cookie-consent manifest). MUST precede the
    // bare `captcha` row so the vendor attribution wins over captcha-CHL.
...
/// FP-Tier1: `akam/13` is the bot-detection bootstrap `<script>` that loads
/// on **every** page fronted by that vendor including fully-benign ones (the
/// classifier's own history documents this over-match). It only
/// indicates a *challenge* when paired with one of these structural
/// challenge markers. Without a co-signal, a small fronted page
/// (e.g. a retailer's "Choose a country" i18n splash, 7.9 KB, akam/13
/// only) must NOT be SensorChallenge-CHL.
...
/// textarea, the `gstatic.com/recaptcha` script (a ~9.6 KB shell /
/// a "browser not supported" page = pure FP, zero anti-bot
...
fn verdict_for(tag: &str, len: usize) -> ChallengeVerdict {
    match tag {
        ...
        // FP-B4: a challenge-vendor block in a *large* body is the
        // orchestrator shell that ran but never cleared (the large-shell
        // class) — ChallengeIncomplete, NOT SensorFail (the body was never
        // replaced with real content, so the sensor did not "score us
        // bot"; mislabeling it SensorFail misdirects work to fingerprint
        // tuning). A small stub stays EdgeBlock (classic edge deny).
...
pub fn engine_classify(body: &str) -> EngineClass {
    ...
    let tag: &'static str = 'tag: {
        ...
        // an active PoW loader. A solved page retains the config vars but
...
/// FP-C2: does this *initial response body* indicate a challenge-vendor
/// challenge / managed-challenge orchestrator? Detected from the initial
/// body so the navigate loop can set a *persistent* `started_as_managed_challenge`
/// origin flag.
/// Without it, the cookie-diff retry / pending-nav poll gate on the
/// *post-mutation* DOM; the orchestrator rewrites the body, so the
/// marker drops while `cf_clearance` was never issued and a
/// body-mutated-but-unsolved challenge page silently slips the retry gate
/// (the mutable-state-guard class). Single source of truth for
/// the origin substrings.
...
mod tests {
    ...
    // (CSS class, analytics key, cookie-consent JSON manifest — a
    // historical FP root on one such site) must classify as a pass, while a real
    ...
    fn fp_b2_literal_strong_markers_size_gated() {
        ...
        // Large-page shape: px-captcha only in a cookie-consent manifest.
        let wf = big(
        ...
        // True detection preserved: a real small anti-bot interstitial
        ...
        // HoldChallenge-PaH phrase and would mask the row this test pins.
        ...
        // And the PaH phrase still classifies as a HoldChallenge-PaH challenge.
    ...
    fn fp_b5_awswaf_markers_loader_cosigned() {
        ...
        // Stub shape: unsolved AWS-WAF stub with the live loader — detected.
    ...
    // FP-B4 regression: a large challenge-vendor orchestrator shell that ran
    // but never cleared (the large-shell class) classifies ChallengeIncomplete —
    // NOT SensorFail (no fingerprint scoring happened) and NOT Pass (no
    // real content). A small stub stays EdgeBlock. A genuinely
    // passed page that merely retains the always-on
    ...
    fn fp_b4_managed_incomplete_split_from_sensorfail() {
        ...
        // Small stub ⇒ EdgeBlock (unchanged classic edge deny).
        let stub =
        ...
        // Passed page: real content + the always-on JSD URL but NO
    ...
    // FP-C2 regression: the persistent origin predicate fires on the
    // challenge / orchestrator shell (so the navigate-loop retry/poll
    ...
    // engine can never *fabricate* a vendor pass. Any challenge body
    ...
    fn fp_d2_managed_unsolved_never_passes() {
        ...
        // Large orchestrator shell (the large-shell class).
    ...
    // challenge (the invisible-v3 shell class), and the bare bootstrap
    // alone is NOT a challenge (the i18n-splash class) — while a
    // real interactive captcha / a co-signalled vendor interstitial
    // still classify as challenges.
    #[test]
    fn fp_t1_invisible_recaptcha_and_akam13_cosignal() {
        // Invisible-v3 shape: ~9.6 KB shell, only invisible v3 plumbing.
        ...
        // i18n-splash shape: ~7.9 KB, only the akam/13 bootstrap, no
        // challenge co-signal ⇒ NOT SensorChallenge-CHL (the i18n splash).
        ...
        // akam/13 WITH a vendor challenge co-signal still → SensorChallenge-CHL.
    ...
    // INVERSE-CHL regression guard: an AWS-WAF token
    ...
    // into a false `Pass`. A solved e-commerce page (no AWS envelope vars)
    // must still be a clean `Pass`.
    #[test]
    fn inverse_chl_awswaf_never_passes_unsolved() {
        // The real ~2 KB AWS-WAF stub shape.
        ...
        // byte count — is what keeps it a challenge (a solved page drops
        ...
        // No false positive: a solved product page (no AWS envelope
    ...
    // TAIL-PIN regression guard: the known small "pass"
    // shells must stay `ThinShell` (not over-counted as full passes) so the
    // the live-nav drain work can't manufacture a measurement-artifact win by simply
    // growing a shell a few KB. One known shell (~13 KB) sits just under the 15 KB
    // floor — the case DUOLINGO-ASSERT protects.
    #[test]
    fn tail_pin_known_thin_shells_stay_thinshell() {
        // A "browser not supported" shell ~13.3 KB, invisible v3 only.
