...
/// inherited `guest_id` + the `__cf_bm` bot-management cookie as a stale "previously-issued
/// session" and serves a 69-byte stub. Scrub BOTH identities, but ONLY when
/// the jar already holds cookies for both (the second-visit poisoning
/// condition — a first twitter nav with an empty jar must not fire).
///
...
async fn scrub_xcom_cookie_collision(url: &str, client: &crate::net::HttpClient) {
    ...
    let is_collision_target =
        h == "x.com" || h.ends_with(".x.com") || h == "twitter.com" || h.ends_with(".twitter.com");
    if !is_collision_target {
        return;
    }
    let t_url = url::Url::parse("https://twitter.com/").ok();
    let x_url = url::Url::parse("https://x.com/").ok();
    let has_twitter = matches!(&t_url, Some(u) if client.cookies_for_url(u).await.is_some());
    let has_x = matches!(&x_url, Some(u) if client.cookies_for_url(u).await.is_some());
...
impl Page {
    ...
    async fn navigate_loop_internal(
        ...
        // 15s baseline:
        //   - PoW-VM-protected: ~530KB ips.js VM that runs PoW + sensor
        //     POST + token negotiation; 25-40s on our V8. Bump to 45s.
        //   - sensor-payload-protected: sensor flow ~20s. Bump to 25s.
            ...
            .as_deref()
        {
            ...
            Some(h)
                if h.ends_with("twitter.com")
                    ...
                    // Give it the SPA tier.
                    || h.ends_with("uber.com") =>
            {
                90_000
            }
            // NOTE: heavy proof-of-work challenge pages are NOT special-cased
            // by host here — they're handled generically below by the
            // `started_as_seccpt_challenge` budget override (env-configurable),
            // which applies to the whole challenge class by marker rather than
            // a hardcoded host list.
            ...
            // sensor-payload tier is too tight for (observed flip at ~70s) — give it 60s.
            Some(h) if h.ends_with("bestbuy.com") => 60_000,
            // sensor-payload-protected (plain sensor flow, no sec-cpt PoW).
            Some(h)
