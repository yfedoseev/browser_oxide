...
use browser_oxide::{Page, ParallelPager};
...
///
/// **Important — what is and isn't a challenge marker.** The bot-detection
/// sensor's `<script src=".../akam/13/<hash>">` bootstrap loads on every
/// protected page (rendered or challenge). Its mere presence in
/// the HTML is NOT a challenge indicator — real Chrome sees the same
/// tag on a successfully-loaded storefront homepage. The actual
/// challenge signal is **content substitution**: the page body is
/// replaced with an interstitial (≤ 30 KB usually). Same for the
/// other inline tracker variable names — these appear in inline analytics
/// JS on plenty of normal pages. The canonical interstitial-vs-content
/// discriminator is **body size** + presence of the marker.
///
/// History: a "strong markers" rule that fired on those marker
/// substrings regardless of body size used to incorrectly
/// flag many large storefront/streaming sites as a challenge page
/// even when the page rendered with multi-MB content.
/// FP-B1: thin wrapper over the single canonical classifier
/// `browser_oxide::classify::engine_classify`. The marker tables and size
...
mod classifier_tests {
    ...
    fn large_rendered_page_with_just_a_moment_phrase_is_not_chl() {
        ...
        // flagged this as a challenge page on every profile, producing a
        // bogus universal-block tally for the site.
