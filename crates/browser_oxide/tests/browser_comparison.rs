...
async fn compare_evaluate_speed() {
    ...
    // browser_oxide
    let server = browser_oxide::protocol::CdpServer::start_ephemeral(
...
async fn compare_navigation() {
    ...
    if let Ok(server) = browser_oxide::protocol::CdpServer::start_navigable(0) {
        tokio::time::sleep(Duration::from_millis(50)).await;
...
struct SiteEntry {
    ...
    #[allow(
        dead_code,
        reason = "documents the site's protection category in the table"
    )]
    category: &'static str,
}
...
fn anti_bot_sites() -> Vec<SiteEntry> {
    vec![
        // Challenge vendor A (bot management)
        SiteEntry {
        ...
        // Challenge vendor C (bot manager)
        SiteEntry {
...
async fn compare_anti_bot_quick() {
    ...
    let mut results = Vec::new();
    for site in &sites {
        match browser_oxide::protocol::CdpServer::start_with_url(site.url, 0) {
            Ok(server) => {
                tokio::time::sleep(Duration::from_millis(100)).await;
...
async fn compare_throughput() {
    ...
    {
        let (mut ok, mut fail) = (0u32, 0u32);
        match browser_oxide::protocol::CdpServer::start_navigable(0) {
            Ok(server) => {
                tokio::time::sleep(Duration::from_millis(50)).await;
...
async fn compare_resource_usage() {
    ...
    {
        let start = Instant::now();
        let server = browser_oxide::protocol::CdpServer::start_navigable(0).unwrap();
...
async fn compare_eventsource_support() {
    ...
    // browser_oxide
    {
        let server = browser_oxide::protocol::CdpServer::start_ephemeral("<html></html>").unwrap();
        tokio::time::sleep(Duration::from_millis(50)).await;
        if let Ok(mut client) = CdpClient::connect(&server.ws_url()).await {
            let _ = client.send("Runtime.enable", json!({})).await;
