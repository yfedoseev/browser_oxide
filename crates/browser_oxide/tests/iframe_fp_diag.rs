...
//! Chrome 147.
//! The challenge vendor's sensor runs FP probes inside a vendor-created child iframe;
//! if our engine's contentWindow realm diverges from real Chrome's, the
//! VM bails before POSTing its report. This test prints exactly what our
...
async fn script_vendor_ifw_deep_probe() {
  ...
  try {
    ...
    println!("SCRIPT-VENDOR-IFW-DEEP-PROBE: {r}");
}
...
async fn script_vendor_frame_index_probe() {
    ...
    .await
    .unwrap();
    // Mirrors the challenge vendor's actual probe: access child iframe via window[0] / frames[0],
  ...
  try {
    ...
    return JSON.stringify({
      ...
      win0_self_eq: (cw0 && cw0.self === cw0),
        ...
        .unwrap_or_else(|e| format!("EVAL_ERR: {e}"));
    println!("SCRIPT-VENDOR-FRAME-INDEX-PROBE: {r}");
}
...
async fn script_vendor_ifw_smc_probe() {
    ...
    .await
    .unwrap();
    // Mirrors the challenge vendor's ifw and smc probe checks exactly:
    //   i_ciw  = cw instanceof Window
  ...
  try {
    ...
    println!("SCRIPT-VENDOR-IFW-SMC-PROBE: {r}");
}
...
async fn script_vendor_spd_probe() {
    ...
    .await
    .unwrap();
    // Mirrors the challenge vendor's spd probe: reads screen/viewport props from child iframe window.
    let r = page
        .evaluate(
            r#"(function(){
  try {
    ...
    return JSON.stringify({
      ...
      dpr: cw.devicePixelRatio !== undefined ? cw.devicePixelRatio : 'n/a',
        ...
        .unwrap_or_else(|e| format!("EVAL_ERR: {e}"));
    println!("SCRIPT-VENDOR-SPD-PROBE: {r}");
}
