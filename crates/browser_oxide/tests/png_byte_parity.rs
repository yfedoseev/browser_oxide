...
//! probes (CreepJS lies-canvas, the challenge vendor's pHash) hash the data URL, so
...
use browser_oxide::canvas::Canvas2D;
// sha2 is only used by the reference-platform (x86_64 Linux) pinned-hash test.
#[cfg(all(target_arch = "x86_64", target_os = "linux"))]
use sha2::{Digest, Sha256};

// Only used by the reference-platform pinned-hash test below; gated with the
// same cfg so it isn't dead code (which `-D warnings` would reject) elsewhere.
#[cfg(all(target_arch = "x86_64", target_os = "linux"))]
...
///
/// Reference-platform only: tiny-skia rasterizes via SIMD whose rounding
/// differs between x86_64 and aarch64, so the exact bytes (and thus the SHA)
/// are pinned for x86_64 Linux. Cross-run determinism — the property that
/// actually matters for canvas-fingerprint stability — is asserted by
/// `png_encoding_is_byte_deterministic` above and is platform-agnostic.
#[cfg(all(target_arch = "x86_64", target_os = "linux"))]
#[test]
