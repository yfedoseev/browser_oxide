...
async fn stealth_profile_wired_end_to_end() {
    ...
    // Chrome major version moves with the preset; pin to the family
    // (Chrome NNN with a /version suffix) rather than a specific
    // release so the test survives quarterly preset bumps.
    assert!(
...
async fn spa_like_render_cycle() {
    ...
    "#), None::<browser_oxide::stealth::StealthProfile>).await.unwrap();
...
async fn e2e_stylesheet_collector_finds_link_tags() {
    ...
    let entries = browser_oxide::stylesheet_collector::find_stylesheets(&dom);
    assert_eq!(
        entries.len(),
        2,
    ...
    assert!(
        matches!(&entries[0], browser_oxide::stylesheet_collector::StylesheetEntry::External(h) if h == "/style.css")
    );
    assert!(
        matches!(&entries[1], browser_oxide::stylesheet_collector::StylesheetEntry::Inline(c) if c.contains("color: green"))
    );
}
