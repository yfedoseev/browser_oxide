use css_parser::{ComponentValue, Token, TokenKind};
...
mod tests {
    ...
    fn eval(css: &str) -> bool {
        ...
        let (stylesheet, _) = css_parser::parse_stylesheet(&input);
        if let Some(css_parser::Rule::At(at)) = stylesheet.rules.first() {
