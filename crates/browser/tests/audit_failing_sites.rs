...
async fn capture_sites(sites: &[(&str, &str, &str)], out_dir: &str) {
    ...
    for (key, bucket, url) in sites {
        ...
        eprintln!("  ok={} body_len={} cookies={} markers={:?} ms={}",
                  record.ok, record.body_len, record.cookie_names.len(),
                  record.html_markers, record.elapsed_ms);
        for ctx in &record.marker_contexts { eprintln!("    ctx: {ctx}"); }
    ...
    eprintln!("\n=== AUDIT DONE — wrote {} records to {out_dir}/", summary.len());
}
...
async fn build_record(
    ...
    match result {
        Ok(mut page) => {
            record.ok = true;
            let body = page.evaluate("document.documentElement.outerHTML").unwrap_or_default();
            let body_clean = body.trim_matches('"');
            record.body_len = body_clean.len();
            record.final_url = page.evaluate("location.href").unwrap_or_default()
            ...
            for marker in ["_abck", "bm_sz", "bm_sv", "ak_bmsc", "_px3", "_pxhd",
                           "datadome", "kpsdk", "cf_clearance", "__cf_bm",
                           "incap_ses_", "__wbauid"] {
                for c in cookies_clean.split(';') {
                    let c = c.trim();
            ...
            for marker in ["px-captcha", "_pxhd", "_px3", "ips.js", "kpsdk",
                           "_abck", "bm_sz", "bot/captcha", "datadome",
                           "just a moment", "checking your browser",
                           "are you a robot", "press &amp; hold",
                           "x-amzn-waf-action", "g-recaptcha", "akam/13",
                           "captcha"] {
                if let Some(idx) = lower.find(marker) {
                    record.html_markers.push(marker.to_string());
                    let start = idx.saturating_sub(40);
                    let end = (idx + marker.len() + 40).min(body_clean.len());
                    let snippet: String = body_clean[start..end]
                        .chars()
                        .take(80)
                        .collect();
                    record.marker_contexts.push(format!("{marker}: …{snippet}…"));
                }
            }
        }
...
async fn audit_all_failing_sites() {
    ...
    eprintln!("\n=== AUDIT DONE — wrote {} site records to {out_dir}/", summary.len());

    // Print a 2-line summary per site to stderr for at-a-glance review.
    for r in &summary {
        eprintln!(
            "  {:<18} {:<18} body={:>8} cookies={:?}",
            r.key, r.vendor_bucket, r.body_len, vendor_cookies(&r.cookie_names)
        );
        if !r.html_markers.is_empty() {
            eprintln!("                                       markers={:?}", r.html_markers);
        }
    }
}

fn vendor_cookies(names: &[String]) -> Vec<String> {
    let interesting = ["_abck", "bm_sz", "bm_sv", "ak_bmsc", "_px3", "_pxhd",
                       "_pxvid", "datadome", "kpsdk", "cf_clearance", "__cf_bm"];
    names
        .iter()
        .filter(|n| interesting.iter().any(|m| n.starts_with(m) || n.contains(m)))
        .cloned()
        .collect()
}
