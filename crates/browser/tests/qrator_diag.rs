...
async fn debug_qrator_dns_shop() {
    ...
    match result {
        Ok(mut page) => {
            ...
            let logs_json: String = page.evaluate("JSON.stringify(globalThis.__scriptErrors || [])").unwrap_or_else(|_| "[]".to_string());
            ...
            let missing: String = page.evaluate("JSON.stringify(window.__missingApis || [])").unwrap_or_else(|_| "[]".to_string());
            println!("\n[MISSING APIs]:\n{}", missing.replace("},{", "}\n{"));

            let wasm: String = page.evaluate("typeof WebAssembly").unwrap_or_else(|_| "error".to_string());
            println!("[WebAssembly]: {}", wasm);

            let intl: String = page.evaluate(r#"
                JSON.stringify({
            ...
            // Extract fetch log
            let fetches: String = page.evaluate(r#"
            ...
            if content.contains("qrator") || content.contains("401") || content.contains("Unauthorized") {
                println!("STILL BLOCKED by QRATOR (or 401)");
            } else {
                println!("SUCCESS? content preview: {}", content.chars().take(200).collect::<String>());
            }
        }
        Err(e) => {
