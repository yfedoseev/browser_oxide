...
mod tests {
    ...
    async fn test_pixelscan_oxide() {
        ...
        let mut page = Page::navigate("https://pixelscan.net/", profile, 5).await.unwrap();
    ...
    async fn test_pixelscan_check_oxide() {
        let profile = stealth::presets::chrome_130_macos();
        let mut page = Page::navigate("https://pixelscan.net/fingerprint-check", profile, 5).await.unwrap();
