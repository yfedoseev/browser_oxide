...
mod tests {
    ...
    async fn test_creepjs_oxide() {
        ...
        let mut page = Page::navigate("https://abrahamjuliot.github.io/creepjs/", profile, 5).await.unwrap();
        
        // Wait for CreepJS to compute
        for _ in 0..10 {
            tokio::time::sleep(std::time::Duration::from_secs(1)).await;
            let ready = page.evaluate("!!document.querySelector('.fingerprint-header-container')").unwrap_or_default();
            if ready == "true" {
                break;
            }
        }
        ...
        let inner_text = page.evaluate("document.body.innerText.substring(0, 1000)").unwrap_or_default();
