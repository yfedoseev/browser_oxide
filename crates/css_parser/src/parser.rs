...
impl<'a> Parser<'a> {
    ...
    /// Peek past whitespace to find the next non-whitespace token kind.
    #[allow(dead_code)]
    fn peek_non_ws(&self, start_offset: usize) -> &TokenKind<'a> {
        let mut i = self.pos + start_offset;
        while i < self.tokens.len() {
            if !matches!(self.tokens[i].kind, TokenKind::Whitespace) {
