...
- **Rust 1.84+** — pinned by `deno_core 0.403` / V8 prebuilts.
...
The whole engine is the single `browser_oxide` crate
...
| `host` | `Send + Sync` handle over the `!Send` engine (binding bridge). |
| crate root | Top-level `Browser` / `Page` API. |

When adding a new file, put it under the module that owns the
...
- **Integration tests** live in `crates/browser_oxide/tests/`.
...
Releases are driven from `main`. Tag with `vX.Y.Z`; `cargo publish`
`browser_oxide` first, then `browser_oxide_mcp`. Don't release with a
red CI.
